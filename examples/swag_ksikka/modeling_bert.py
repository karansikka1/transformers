...
class BertForMultipleChoice(BertPreTrainedModel):
    def __init__(self, config, **kwargs):
        ...
        self.knowledge_method = kwargs["knowledge_method"]

        if self.knowledge_method == 1:
            cluster_embeddings, concepts = pickle.load(
                open(kwargs["cluster_path"], "rb")
            )
            self.knowledge_emb = nn.Parameter(torch.Tensor(cluster_embeddings))
            self.concepts = concepts
            emb_dim = cluster_embeddings.shape[1]
            # Attention from Pooled node to knowledge
            self.att_layer = nn.Linear(config.hidden_size, emb_dim)
            # Projection to
            self.projection_to_hidden_space = nn.Linear(emb_dim, config.hidden_size)
    ...
    Examples::
        ...
        if self.knowledge_method == 1:
            att_scores = torch.matmul(
                self.att_layer(pooled_output), self.knowledge_emb.transpose(1, 0)
            )
            att_scores = nn.functional.softmax(att_scores, 1)
            pooled_knowledge_tuples = self.projection_to_hidden_space(
                att_scores.matmul(self.knowledge_emb)
            )
            pooled_output_new = pooled_output + pooled_knowledge_tuples
        elif self.knowledge_method == 0:
            pooled_output_new = pooled_output

        pooled_output = self.dropout(pooled_output_new)
        logits = self.classifier(pooled_output)
